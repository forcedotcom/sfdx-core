        ...
        try {
            ...
            if (err && err.code === 'ENOENT') {
                ...
                if (indexOflastSlash > 0) {
                    ...
                    throw almError('InvalidProjectWorkspace');
                }
            }
        }
    };
...
const workspace = {
    ...
    resolveConfigContent(projectDir) {
        ...
        return workspace.getGlobalConfig(_WORKSPACE_CONFIG_FILENAME)
        .then(configObject => {
        ...
        .then(exist => {
            if (!exist) {
                throw almError('MissingAppConfig');
            }
    ...
    getOauthLocalPort() {
        ...
        if (Number.isNaN(configPort) || configPort < 1 || configPort > (Math.pow(2, 16) - 1)) {
            throw almError('InvalidOAuthRedirectUrlPort', configPort);
        }
        return configPort;
    },
    ...
    /**
     * Save a new scratch org as the workspace org in the workspace org config
     * file within the local state folder. This will replace the whole file with
     * the new scratch org.
     * @param {string} scratchOrgName the name of the new scratch org
     * @returns {Promise} resolved when the file is saved
     */
    saveWorkspaceOrg(scratchOrgName) {
        if (util.isNullOrUndefined(scratchOrgName)) {
            throw almError('MissingRequiredParameter', scratchOrgName);
        }

        // First load the config so we don't replace any values
        return workspace.resolveConfig().then(config => {
            config.scratchOrgName = scratchOrgName;

            const filePath = workspace.getWorkspaceOrgConfigPath();
            // file is customer-editable, so write w/ spaces for readability
            const fileData = JSON.stringify({ scratchOrgName }, null, 4);
            const fileOpts = { flag: 'w', encoding: 'utf-8' };

            return workspace.ensureDirectoryExists(workspace.getWorkspaceStateFolderPath())
            .then(() => fs.writeFileAync(filePath, fileData, fileOpts));
        });
    },
    ...
    readJSON(jsonPath) {
        ...
        .then(data => {
            if (!data.length) {
                throw almError('JsonParseError', [jsonPath, 1, 'FILE HAS NO CONTENT']);
            }
            fileContents = data;
            return data;
        })
        ...
        .catch(err => {
            if (err.name === 'SyntaxError') {
                ...
                throw almError('JsonParseError', [jsonPath, lineNumber, errorPortion]);
            } else {
                throw err;
            }
        });
    ...
    getStateFolderPath(wsPath) {
        if (!wsPath) {
            wsPath = workspace.getProjectPath();
        }
        return path.join(workspace.getProjectPath(), workspace.getWorkspaceStateFolderName());
    },
    ...
    getStateFilePath(jsonConfigFileName) {
        if (util.isNullOrUndefined(jsonConfigFileName)) {
            throw almError('MissingRequiredParameter', jsonConfigFileName);
        }
        return path.join(workspace.getProjectPath(), workspace.getWorkspaceStateFolderName(), jsonConfigFileName);
    },
    ...
    getWorkspaceOrgConfigPath(wsPath) {
        return path.join(workspace.getWorkspaceStateFolderPath(wsPath), 'workspaceorg.json');
    },
    ...
    saveGlobalConfig(jsonConfigFileName, jsonConfigObject) {
        if (util.isNullOrUndefined(jsonConfigObject)) {
            throw almError('MissingRequiredParameter', 'jsonConfigObject');
        }
        ...
        return workspace.resolveStateFolder(workspace.getHomeDir())
