...
function AppCloudCrypto () {
    ...
    this.enableTokenEncryption = util.isNullOrUndefined(config.EnableTokenEncryption) ?
        true :
        config.EnableTokenEncryption;
}
    ...
    return KeychainPromises.get('appcloud', 'local').then((savedKey) => {
        ...
        if (!util.isNullOrUndefined(err) && err.name  === 'PasswordNotFound') {
            ...
            if (!util.isNullOrUndefined(retried)) {
                throw new Error(`${messages.getMessage('keyChainItemCreateFailed')}: ${err.message}`);
            }
            ...
            return KeychainPromises.set('appcloud', 'local', key)
                .then(() => this.init(true))
                .catch((err2) => {
                    throw new Error(`${messages.getMessage('keyChainItemCreateFailed')}: ${err2.message}`);
                });
        }
        else {
            throw err;
