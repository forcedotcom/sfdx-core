...
class Force {
     ...
     * @param oauthConfig
     * @param type - the org type e.g. hub, development, test, etc.
     ...
     */
     authorizeAndSave(oauthConfig, type) {
        ...
        if (!util.isNullOrUndefined(oauthConfig.privateKeyFile)) {
            promise = Force.jwtAuthorize(oauthConfig);
        } else {
            isJwt = false;
            promise = Force.authorize(oauthConfig);
        }
            ...
            .then(connection => Promise.join(
                ...
                (describeResponse, permSets) => {
                    if (!permSets.records || permSets.records.length !== 1) {
                        throw sfdxError('PermissionSetNotFoundError', [permSetName]);
                    }
