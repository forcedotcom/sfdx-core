...
class Messages {
  ...
  static importMessagesDirectory(moduleDirectoryPath, truncateToProjectPath = true, packageName) {
    ...
    if (!packageName) {
      ...
      try {
        packageName = index.lib.asString(
          index.lib.ensureJsonMap(Messages._readFile(path.join(moduleMessagesDirPath, 'package.json'))).name
        );
        if (!packageName) {
          throw new index$1.lib.NamedError('MissingPackageName', errMessage);
        }
      } catch (err) {
        throw new index$1.lib.NamedError('MissingPackageName', errMessage, err);
      }
    }
  ...
  static loadMessages(packageName, bundleName) {
    ...
    throw new index$1.lib.NamedError(
      'MissingBundleError',
  ...
  getMessageWithMap(key, tokens = [], map) {
    ...
    if (group) {
      ...
      if (childObject && index.lib.isAnyJson(childObject)) {
    ...
    const messages = index.lib.isArray(msg) ? msg : [msg];
    return messages
      .map(message => {
        index.lib.ensureString(message);
