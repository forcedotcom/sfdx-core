...
class SfdxErrorConfig {
   ...
   */
  load() {
    this.messages = messages.Messages.loadMessages(this.packageName, this.bundleName);
 ...
 * ```
 */
class SfdxError extends index.lib.NamedError {
  // The create implementation function.
  static create(nameOrConfig, bundleName, key, tokens) {
    let errorConfig;
    if (index$1.lib.isString(nameOrConfig)) {
      errorConfig = new SfdxErrorConfig(nameOrConfig, index$1.lib.ensure(bundleName), index$1.lib.ensure(key), tokens);
    } else {
  ...
  static wrap(err) {
    ...
    if (index$1.lib.hasString(err, 'code')) {
