...
var config_sandboxOrgConfig = require('./config/sandboxOrgConfig.js');
 ...
 */
class Org extends index.lib.AsyncCreatable {
  /**
   * @ignore
   */
  constructor(options) {
  ...
  async cleanLocalOrgData(orgDataPath, throwWhenRemoveFails = false) {
    let dataPath;
    try {
      const rootFolder = await config_config.Config.resolveRootFolder(false);
      dataPath = path.join(rootFolder, global.Global.STATE_FOLDER, orgDataPath ? orgDataPath : 'orgs');
    ...
    return this.manageDelete(async () => await util_fs.fs.remove(dataPath), dataPath, throwWhenRemoveFails);
  }
  /**
   * @ignore
   */
  async retrieveOrgUsersConfig() {
    return await config_orgUsersConfig.OrgUsersConfig.create(
      config_orgUsersConfig.OrgUsersConfig.getOptions(this.getOrgId())
    );
  }
  /**
  ...
  async checkScratchOrg(devHubUsernameOrAlias) {
    ...
    if (!aliasOrUsername) {
      aliasOrUsername = index$1.lib.asString(
        this.configAggregator.getPropertyValue(config_config.Config.DEFAULT_DEV_HUB_USERNAME)
      );
    }
    ...
    const trimmedId = util_sfdc.sfdc.trimTo15(thisOrgAuthConfig.orgId);
    ...
    try {
      ...
      if (err.name === 'INVALID_TYPE') {
        throw sfdxError.SfdxError.create('@salesforce/core', 'org', 'NotADevHub', [devHubConnection.getUsername()]);
      }
      throw err;
    }
    if (index$1.lib.getNumber(results, 'records.length') !== 1) {
      throw new sfdxError.SfdxError('No results', 'NoResults');
    }
  ...
  async getDevHubOrg() {
    if (this.isDevHubOrg()) {
      ...
      const devHubUsername = index$1.lib.ensureString(this.getField(Org.Fields.DEV_HUB_USERNAME));
      return Org.create({
        connection: await connection.Connection.create({
          authInfo: await authInfo.AuthInfo.create({ username: devHubUsername })
        })
      });
    }
  }
  ...
  isDevHubOrg() {
    ...
    if (index$1.lib.isBoolean(isDevHub)) {
      return isDevHub;
    } else {
      return false;
    }
  ...
  async determineIfDevHubOrg(forceServerCheck = false) {
    ...
    if (!forceServerCheck && index$1.lib.isBoolean(cachedIsDevHub)) {
    ...
    const username = index$1.lib.ensure(this.getUsername());
    const auth = await authInfo.AuthInfo.create({ username });
    await auth.save({ isDevHub });
    authInfo.AuthInfo.clearCache(username);
    // Reset the connection with the updated auth file
    this.connection = await connection.Connection.create({
  ...
  async readUserAuthFiles() {
    ...
    const thisUsername = index$1.lib.ensure(this.getUsername());
    const usernames = index$1.lib.ensureJsonArray(contents.usernames || [thisUsername]);
    return Promise.all(
      usernames.map(username => {
        if (username === thisUsername) {
          return authInfo.AuthInfo.create({
            username: this.getConnection().getUsername()
          });
        } else {
          return authInfo.AuthInfo.create({ username: index$1.lib.ensureString(username) });
        }
      })
    );
  }
  ...
  async addUsername(auth) {
    if (!auth) {
      throw new sfdxError.SfdxError('Missing auth info', 'MissingAuthInfo');
    }
    const _auth = index$1.lib.isString(auth) ? await authInfo.AuthInfo.create({ username: auth }) : auth;
    ...
    if (!index$1.lib.isArray(usernames)) {
      throw new sfdxError.SfdxError('Usernames is not an array', 'UnexpectedDataFormat');
    }
   ...
   */
  async init() {
    this.logger = await logger.Logger.child('Org');
    this.configAggregator = this.options.aggregator
      ? this.options.aggregator
      : await config_configAggregator.ConfigAggregator.create();
    if (!this.options.connection) {
      if (this.options.aliasOrUsername == null) {
        ...
        const aliasOrUsername = this.options.isDevHub
          ? index$1.lib.getString(this.configAggregator.getInfo(config_config.Config.DEFAULT_DEV_HUB_USERNAME), 'value')
          : index$1.lib.getString(this.configAggregator.getInfo(config_config.Config.DEFAULT_USERNAME), 'value');
      ...
      this.connection = await connection.Connection.create({
        ...
        authInfo: await authInfo.AuthInfo.create({
          username: (username != null && (await config_aliases.Aliases.fetch(username))) || username
        })
      });
    } else {
   ...
   */
  getDefaultOptions() {
    throw new sfdxError.SfdxError('Not Supported');
  }
  /**
  ...
  async retrieveSandboxOrgConfig() {
    return await config_sandboxOrgConfig.SandboxOrgConfig.create(
      config_sandboxOrgConfig.SandboxOrgConfig.getOptions(this.getOrgId())
    );
  }
  ...
  async removeUsers(throwWhenRemoveFails) {
    ...
    if (await config.exists()) {
      ...
      const aliases = await config_aliases.Aliases.create(config_aliases.Aliases.getDefaultOptions());
      ...
      for (const auth of _auths) {
        ...
        if (username === this.getUsername()) {
          ...
          const _info = await authInfo.AuthInfo.create({ username });
          const connection$1 = await connection.Connection.create({ authInfo: _info });
          orgForUser = await Org.create({ connection: connection$1 });
        }
        const orgType = this.isDevHubOrg()
          ? config_config.Config.DEFAULT_DEV_HUB_USERNAME
          : config_config.Config.DEFAULT_USERNAME;
        ...
        if (
          ...
          await config_config.Config.update(configInfo.isGlobal(), orgType, undefined);
        }
