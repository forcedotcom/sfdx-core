 ...
 */
export class Org extends AsyncCreatable {
  /**
   * @ignore
   */
  constructor(options) {
  ...
  async readUserAuthFiles() {
    ...
    const usernames = ensureJsonArray(contents.usernames || [thisUsername]);
    return Promise.all(
    ...
    if (await config.exists()) {
      ...
      for (const auth of _auths) {
        ...
        if (username === this.getUsername()) {
          ...
          const connection = await Connection.create({ authInfo: _info });
