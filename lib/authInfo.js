...
var index$2 = require('./index-ffe6ca9f.js');
...
var index = require('./index-e6d82ffe.js');
...
var Buffer$3 = driver.safeBuffer.Buffer;
...
var Buffer$4 = driver.safeBuffer.Buffer;
...
var tostring = function toString(obj) {
  ...
  if (typeof obj === 'number' || Buffer$5.isBuffer(obj)) return obj.toString();
  return JSON.stringify(obj);
};

/*global module*/

var Buffer$6 = driver.safeBuffer.Buffer;

function base64url(string, encoding) {
  return Buffer$6.from(string, encoding)
    .toString('base64')
    .replace(/=/g, '')
    .replace(/\+/g, '-')
    .replace(/\//g, '_');
...
var Buffer$7 = driver.safeBuffer.Buffer;
...
var timespan = function(time, iat) {
  ...
  if (typeof time === 'string') {
    var milliseconds = index.ms(time);
...
var verify$1 = function(jwtString, secretOrPublicKey, options, callback) {
  ...
  if (typeof secretOrPublicKey === 'function') {
    if (!callback) {
      return done(
        new JsonWebTokenError_1('verify must be called asynchronous if secret or public key is provided as a callback')
      );
    }
...
function isBoolean(value) {
  return value === true || value === false || (isObjectLike$1(value) && objectToString$1.call(value) == boolTag);
}
...
/** Used as references for various `Number` constants. */
var INFINITY$1 = 1 / 0,
  MAX_INTEGER$1 = 1.7976931348623157e308,
  NAN$1 = 0 / 0;
...
function isPlainObject(value) {
  if (!isObjectLike$4(value) || objectToString$4.call(value) != objectTag || isHostObject(value)) {
    return false;
  }
...
/** Used as references for various `Number` constants. */
var INFINITY$2 = 1 / 0,
  MAX_INTEGER$2 = 1.7976931348623157e308,
  NAN$2 = 0 / 0;
...
var sign_options_schema = {
  expiresIn: {
    isValid: function(value) {
      return lodash_isinteger(value) || (lodash_isstring(value) && value);
    },
  ...
  audience: {
    isValid: function(value) {
      return lodash_isstring(value) || Array.isArray(value);
    },
    message: '"audience" must be a string or array'
  },
  algorithm: {
    isValid: lodash_includes.bind(null, SUPPORTED_ALGS),
  ...
  Object.keys(options_to_payload).forEach(function(key) {
    ...
    if (typeof options[key] !== 'undefined') {
      if (typeof payload[claim] !== 'undefined') {
        return failure(
          new Error('Bad "options.' + key + '" option. The payload already has an "' + claim + '" property.')
        );
      }
...
class JwtOAuth2 extends connection.jsforce.OAuth2 {
  constructor(options) {
    super(options);
  }
 ...
 */
class OAuth2WithVerifier extends connection.jsforce.OAuth2 {
  constructor(options) {
    super(options);
    // Set a code verifier string for OAuth authorization
    this.codeVerifier = base64UrlEscape(crypto.randomBytes(Math.ceil(128)).toString('base64'));
  }
  /**
  ...
  getAuthorizationUrl(params) {
    ...
    index$1.lib.set(params, 'code_challenge', codeChallenge);
  ...
  async _postParams(params, callback) {
    index$1.lib.set(params, 'code_verifier', this.codeVerifier);
    // @ts-ignore TODO: need better typings for jsforce
...
})(exports.SfdcUrl || (exports.SfdcUrl = {}));
...
function getJwtAudienceUrl(options) {
  ...
  let audienceUrl = exports.SfdcUrl.PRODUCTION;
  const loginUrl = index$2.lib.getString(options, 'loginUrl', '');
  const createdOrgInstance = index$2.lib
    .getString(options, 'createdOrgInstance', '')
    .trim()
    .toLowerCase();
  if (process.env.SFDX_AUDIENCE_URL) {
    ...
    audienceUrl = exports.SfdcUrl.SANDBOX;
...
class AuthInfoCrypto extends crypto$1.Crypto {
  ...
  _crypt(fields, method) {
    const copy = {};
    for (const key of index$2.lib.keysOf(fields)) {
      const rawValue = fields[key];
      if (rawValue !== undefined) {
        if (index$2.lib.isString(rawValue) && AuthInfoCrypto.encryptedFields.includes(key)) {
          copy[key] = this[method](index$2.lib.asString(rawValue));
        } else {
          copy[key] = rawValue;
        }
      }
 ...
 * ```
 */
class AuthInfo extends index$1.lib.AsyncCreatable {
  /**
   * Constructor
  ...
  static async listAllAuthFiles() {
    const globalFiles = await util_fs.fs.readdir(global.Global.DIR);
    ...
    if (index$1.lib.isEmpty(authFiles)) {
      const errConfig = new sfdxError.SfdxErrorConfig('@salesforce/core', 'core', 'NoAuthInfoFound');
  ...
  static async hasAuthentications() {
    try {
      ...
      return !index$1.lib.isEmpty(authFiles);
    } catch (err) {
  ...
  static getAuthorizationUrl(options) {
    ...
    const params = {
      state: crypto.randomBytes(Math.ceil(6)).toString('hex'),
      prompt: 'login',
  ...
  async save(authData) {
    ...
    const username = index$2.lib.ensure(this.getUsername());
    AuthInfo.cache.set(username, this.fields);
    const dataToSave = index$1.lib.cloneJson(this.fields);
    this.logger.debug(dataToSave);
    const config = await config_authInfoConfig.AuthInfoConfig.create(
      Object.assign({}, config_authInfoConfig.AuthInfoConfig.getOptions(username), { throwOnNotFound: false })
    );
  ...
  update(authData, encrypt = true) {
    if (authData && index$2.lib.isPlainObject(authData)) {
      let copy = index$1.lib.cloneJson(authData);
      if (encrypt) {
  ...
  getSfdxAuthUrl() {
    ...
    const instanceUrl = index$2.lib.ensure(decryptedFields.instanceUrl).replace(/^https?:\/\//, '');
  ...
  async init() {
    ...
    if (!this.options.username && !(this.options.oauth2Options || this.options.accessTokenOptions)) {
      throw sfdxError.SfdxError.create('@salesforce/core', 'core', 'AuthInfoCreationError');
    }
    ...
    if (this.options.username && this.options.oauth2Options) {
      const authInfoConfig = await config_authInfoConfig.AuthInfoConfig.create(
        Object.assign({}, config_authInfoConfig.AuthInfoConfig.getOptions(this.options.username), {
          throwOnNotFound: false
        })
      );
      if (await authInfoConfig.exists()) {
        throw sfdxError.SfdxError.create(
          new sfdxError.SfdxErrorConfig(
            '@salesforce/core',
            'core',
    ...
    this.fields.username = this.options.username || index$2.lib.getString(options, 'username') || undefined;
    // If the username is an access token, use that for auth and don't persist
    const accessTokenMatch =
      index$2.lib.isString(this.fields.username) && this.fields.username.match(/^(00D\w{12,15})![\.\w]*$/);
    if (accessTokenMatch) {
      // Need to initAuthOptions the logger and authInfoCrypto since we don't call init()
      this.logger = await logger.Logger.child('AuthInfo');
      this.authInfoCrypto = await AuthInfoCrypto.create({
        noResetOnClose: true
      });
      const aggregator = await config_configAggregator.ConfigAggregator.create();
      const instanceUrl = aggregator.getPropertyValue('instanceUrl') || exports.SfdcUrl.PRODUCTION;
      this.update({
  ...
  async initAuthOptions(options) {
    ...
    let authConfig;
    if (options) {
      options = index$1.lib.cloneJson(options);
      if (this.isTokenOptions(options)) {
        ...
        if (this.options.parentUsername) {
          ...
          if (process.env.SFDX_CLIENT_SECRET) {
            ...
            Object.assign(options, {
              ...
              privateKey: parentFields.privateKey ? path.resolve(parentFields.privateKey) : parentFields.privateKey
            });
          }
        }
        // jwt flow
        ...
        if (!options.privateKey && options.privateKeyFile) {
          options.privateKey = path.resolve(options.privateKeyFile);
        }
      ...
      authConfig = await this.loadAuthFromConfig(index$2.lib.ensure(this.getUsername()));
  ...
  async loadAuthFromConfig(username) {
    if (AuthInfo.cache.has(username)) {
      return index$2.lib.ensure(AuthInfo.cache.get(username));
    } else {
      // Fetch from the persisted auth file
      try {
        const config = await config_authInfoConfig.AuthInfoConfig.create(
          Object.assign({}, config_authInfoConfig.AuthInfoConfig.getOptions(username), { throwOnNotFound: true })
        );
        return config.toObject();
      } catch (e) {
        if (e.code === 'ENOENT') {
          throw sfdxError.SfdxError.create('@salesforce/core', 'core', 'NamedOrgNotFound', [username]);
        } else {
          throw e;
        }
      }
  ...
  async refreshFn(conn, callback) {
    ...
    try {
      ...
      if (err.message && err.message.includes('Data Not Available')) {
        const errConfig = new sfdxError.SfdxErrorConfig('@salesforce/core', 'core', 'OrgDataNotAvailableError', [
          this.getUsername()
        ]);
        for (let i = 1; i < 5; i++) {
          errConfig.addAction(`OrgDataNotAvailableErrorAction${i}`);
        }
        return await callback(sfdxError.SfdxError.create(errConfig));
      }
  ...
  async buildJwtConfig(options) {
    const privateKeyContents = await util_fs.fs.readFile(index$2.lib.ensure(options.privateKey), 'utf8');
    ...
    let _authFields;
    try {
      _authFields = index$2.lib.ensureJsonMap(await oauth2.jwtAuthorize(jwtToken));
    } catch (err) {
      throw sfdxError.SfdxError.create('@salesforce/core', 'core', 'JWTAuthError', [err.message]);
    }
    const authFields = {
      accessToken: index$2.lib.asString(_authFields.access_token),
      orgId: _parseIdUrl(index$2.lib.ensureString(_authFields.id)).orgId,
      loginUrl: options.loginUrl,
      privateKey: options.privateKey,
      clientId: options.clientId
    };
    const instanceUrl = index$2.lib.ensureString(_authFields.instance_url);
    ...
    try {
      ...
      await this.lookup(index$2.lib.ensure(parsedUrl.hostname));
  ...
  async buildRefreshTokenConfig(options) {
    ...
    const oauth2 = new connection.jsforce.OAuth2(options);
    let _authFields;
    try {
      _authFields = await oauth2.refreshToken(index$2.lib.ensure(options.refreshToken));
    } catch (err) {
      throw sfdxError.SfdxError.create('@salesforce/core', 'core', 'RefreshTokenAuthError', [err.message]);
    }
    return {
   ...
   */
  async exchangeToken(options, oauth2 = new connection.jsforce.OAuth2(options)) {
    // Exchange the auth code for an access token and refresh token.
    let _authFields;
    try {
      this.logger.info(`Exchanging auth code for access token using loginUrl: ${options.loginUrl}`);
      _authFields = await oauth2.requestToken(index$2.lib.ensure(options.authCode));
    } catch (err) {
      throw sfdxError.SfdxError.create('@salesforce/core', 'core', 'AuthCodeExchangeError', [err.message]);
    }
    // @ts-ignore TODO: need better typings for jsforce
    ...
    if (!username) {
      ...
      const instance = index$2.lib.ensure(index$2.lib.getString(_authFields, 'instance_url'));
      const url = `${instance}/services/data/${apiVersion}/sobjects/User/${userId}`;
      const headers = Object.assign(
        { Authorization: `Bearer ${_authFields.access_token}` },
        connection.SFDX_HTTP_HEADERS
      );
      try {
        this.logger.info(`Sending request for Username after successful auth code exchange to URL: ${url}`);
        const response = await new connection.Transport().httpRequest({ url, headers });
        username = index$2.lib.asString(index$1.lib.parseJsonMap(response.body).Username);
      } catch (err) {
        throw sfdxError.SfdxError.create('@salesforce/core', 'core', 'AuthCodeUsernameRetrievalError', [
          orgId,
          err.message
        ]);
      }
