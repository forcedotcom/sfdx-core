...
const sfdc = {
  ...
  isSalesforceDomain: urlString => {
    ...
    return allowlistOfSalesforceDomainPatterns.some(pattern => {
      return url$1.hostname.endsWith(pattern) || allowlistOfSalesforceHosts.includes(url$1.hostname);
    });
  },
  /**
  ...
  findUpperCaseKeys: (data, sectionBlocklist = []) => {
    let key;
    index.lib.findKey(data, (val, k) => {
      if (k[0] === k[0].toUpperCase()) {
        key = k;
      } else if (index$1.lib.isJsonMap(val)) {
        if (sectionBlocklist.includes(k)) {
          return key;
        }
        key = sfdc.findUpperCaseKeys(index$1.lib.asJsonMap(val));
      }
      return key;
    });
    return key;
