...
import { asJsonArray, asNumber, ensure, ensureJsonMap, ensureString, getString, isJsonMap } from '@salesforce/ts-types';
import { EOL } from 'os';
...
async function _retrieveUserFields(username) {
  ...
  if (result.totalSize === 1) {
    const results = mapKeys(result.records[0], (value, key) => lowerFirst(key));
    const fields = {
 ...
 */
export class User extends AsyncCreatable {
  /**
  ...
  async assignPermissionSets(id, permsetNames) {
    ...
    const assignments = await PermissionSetAssignment.init(this.org);
  ...
  async createUser(fields) {
    ...
    const newUserAuthInfo = await AuthInfo.create({
