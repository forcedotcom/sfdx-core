...
describe('Messages', () => {
  ...
  describe('getMessage', () => {
    ...
    it('should return the proper messages', () => {
      ...
      expect(messages.getMessage('msg2', ['blah', 864])).to.equal(
    ...
    it('should return nested messages', () => {
      ...
      expect(messages.getMessage('msg3.msg2', ['blah', 864])).to.equal(
        'test message 2 blah and 864'
      );
      expect(messages.getMessage('msg3.msg3.msg1')).to.equal(testMessages.msg1);
      expect(messages.getMessage('msg3.msg3.msg2', ['blah', 864])).to.equal(
        'test message 2 blah and 864'
      );
    });
    ...
    it('should throw an error if the message is not found', () => {
      try {
        ...
        assert.fail(
          'should have thrown an error that the message was not found'
        );
      } catch (err) {
        expect(err.message).to.equal(
          'Missing message myBundle:msg4 for locale en_US.'
        );
      }
    });
  ...
  describe('importMessageFile', () => {
    it('should throw when file extension is not json', () => {
      try {
        ...
        assert.fail(
          'should have thrown an error that only json files are allowed.'
        );
      } catch (err) {
        expect(err.message).to.contain(
          'Only json and js message files are allowed, not .txt'
        );
      }
    });
    ...
    it('should add the json message file to the map of loaders', () => {
      ...
      $$.SANDBOX.stub(Messages, 'generateFileLoaderFunction').returns(
        'loaderFunction'
      );
  ...
  describe('importMessagesDirectory', () => {
    ...
    const messagesDirPath = `${path.sep}root${path.sep}myModule${path.sep}dist${
      path.sep
    }lib`;
    ...
    beforeEach(() => {
      ...
      statSyncStub.callsFake(statPath => {
        if (
          !statPath.match(/messages/) &&
          statPath !== `${truncatePath}${path.sep}package.json`
        ) {
          throw truncateErr;
        }
    ...
    it('should import each message file', () => {
      ...
      const expectedMsgDirPath =
        path.sep + path.join('root', 'myModule', 'dist', 'lib', 'messages');
      ...
      expect(importMessageFileStub.firstCall.args[1]).to.equal(
        path.join(expectedMsgDirPath, msgFiles[0])
      );
      expect(importMessageFileStub.secondCall.args[0]).to.equal('pname');
      expect(importMessageFileStub.secondCall.args[1]).to.equal(
        path.join(expectedMsgDirPath, msgFiles[1])
      );
    });
    ...
    it('should remove the "/dist" from the module dir path', () => {
      ...
      const expectedMsgDirPath =
        path.sep + path.join('root', 'myModule', 'messages');
      expect(readdirSyncStub.firstCall.args[0]).to.equal(expectedMsgDirPath);
      expect(importMessageFileStub.firstCall.args[1]).to.equal(
        path.join(expectedMsgDirPath, msgFiles[0])
      );
    ...
    it('should remove the "/lib" from the module dir path if there is not dist', () => {
      Messages.importMessagesDirectory(
        messagesDirPath.replace(`${path.sep}dist`, '')
      );
      const expectedMsgDirPath =
        path.sep + path.join('root', 'myModule', 'messages');
      expect(readdirSyncStub.firstCall.args[0]).to.equal(expectedMsgDirPath);
  ...
  describe('generateFileLoaderFunction', () => {
    it('should throw an error when the file does not exist', async () => {
      const loaderFn = Messages.generateFileLoaderFunction(
        'myPluginMessages',
        'myPluginMessages.json'
      );

      try {
        await loaderFn(Messages.getLocale());
        assert.fail(
          'should have thrown an error that the message file was not found.'
        );
      } catch (err) {
    ...
    it('should throw an error when the file is empty', () => {
      ...
      try {
        ...
        expect(err.message).to.equal(
          'Invalid message file: myPluginMessages.json. No content.'
        );
      }
    });
    ...
    it('should throw an error when the file is invalid JSON', () => {
      ...
      try {
        loaderFn(Messages.getLocale());
        assert.fail(
          'should have thrown an error that the file not valid JSON.'
        );
      } catch (err) {
    ...
    it('should return a Messages object', () => {
      ...
      expect(messages.getMessage('msg1')).to.equal(testMessages.msg1);
      expect(messages.getMessage('msg2', ['token1', 222])).to.equal(
        'test message 2 token1 and 222'
      );
    });
  });
  ...
  describe('loadMessages', () => {
    it('should return a cached bundle', async () => {
      const spy = $$.SANDBOX.spy(
        () => new Messages('myBundle', Messages.getLocale(), msgMap)
      );
    ...
    it('should load and return a bundle not in cache', async () => {
      ...
      expect(messages.getMessage('otherMsg1')).to.equal(
        otherMsgMap.get('otherMsg1')
      );
    });
    ...
    it('should throw an error if the bundle is not found', async () => {
      try {
        ...
        assert.fail(
          'should have thrown an error that the bundle was not found.'
        );
      } catch (err) {
        expect(err.message).to.equal(
          'Missing bundle pname:notfound for locale en_US.'
        );
      }
    });
  });
