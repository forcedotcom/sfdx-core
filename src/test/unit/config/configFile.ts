...
import {Config, ConfigOptions} from '../../../lib/config/configFile';
...
class TestConfig extends Config {
    ...
    public static getOptions(filename: string, isGlobal: boolean, isState?: boolean, filePath?: string): ConfigOptions {
        return {
            filename,
            isGlobal,
            isState,
...
describe('Config', () => {
    describe('instantiation', () => {
        it('not using global has project dir', async () => {
            const config: Config = await TestConfig.create(TestConfig.getOptions('test', false));
